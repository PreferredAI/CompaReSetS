...
if __name__ == '__main__':
    ...
    if args.target is not None:
        ...
        output_path = os.path.join(args.input_dir, 'result_{}_{}.csv'.format(begin_index, end_index))
    results = []
    for inc, target_item in tqdm(enumerate(data.keys())):
        if inc >= begin_index and inc <= end_index:
            i_result = pd.read_csv(os.path.join(args.input_dir, '/'.join(str(target_item)), 'evaluation.csv'))
            ...
            results.append({
                ...
                "rouge_1_target": i_result[i_result['item_i']==str(target_item)]['rouge_1'].mean(),
                "rouge_2_target": i_result[i_result['item_i']==str(target_item)]['rouge_2'].mean(),
                "rouge_l_target": i_result[i_result['item_i']==str(target_item)]['rouge_l'].mean(),
            })
